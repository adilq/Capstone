...
class CommNode(Node):
    ...
    def update_waypoints(self):
        odom_pose = self.odom_pose.pose.position
        odom_pose = np.array([odom_pose.x, odom_pose.y])

        target_pose = self.box_pose.pose.position
        target_pose = np.array([target_pose.x, target_pose.y])

        distance = np.linalg.norm(target_pose - odom_pose)
        num_steps = round(distance / NAV_THRESHOLD)
        steps = np.linspace(odom_pose, target_pose, num_steps+1)
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        if MODE == CONNECT:
            ...
            cmd.pose.position.x = node.box_pose.position.x
            cmd.pose.position.y = node.box_pose.position.y
